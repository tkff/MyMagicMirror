...
Module.register("MMM-Hsl-stops", {
    ...
    defaults: {
        ...
        firstLoad: true,
        apiKey: "", // Required: Your Digitransit API key
        stopId: "", // Required: The GTFS ID of the stop
        timeRange: 7200, // Time range in seconds to fetch departures for
        maxFetchedDepartures: 20, // Maximum number of departures to fetch
        maxListedDepartures: 10, // Maximum number of departures to display
        timeToStop: 5, // Time in minutes to reach the stop
        routeIdFilter: [], // Array of route IDs to filter
        stopNickName: "", // Optional nickname for the stop
        debug: false
    },
    ...
    getDom: function () {
        ...
        // Check if stop ID is set
        if (!this.config.stopId) {
            wrapper.innerHTML = self.translate("SETSTOPID") + this.name + ".";
            return wrapper;
        }
        ...
        // Show data if received
        if (stopRoutes && stopRoutes.length > 0 && this.loaded) {
            ...
            Object.keys(stopTimes).slice(0, self.config.maxListedDepartures).forEach(function (key) {
                ...
                if (stopTimes[key].trip.alerts && stopTimes[key].trip.alerts.alertHeaderText !== undefined) {
    ...
    getTableCaption: function () {
        ...
        for (var key in stopRoutes) {
            ...
            if (this.config.routeIdFilter.includes(stopRoutes[key].shortName) || (this.isEmpty(this.config.routeIdFilter))) {
                route.className = "route bright";
            } else {
                route.className = "route dimmed";
            }
    ...
    // Format time-fields used in table
